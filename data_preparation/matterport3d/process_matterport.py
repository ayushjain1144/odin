...
def load_raw_data(
        poses_path, depths_path,
        rgbs_path, intrinsics_path,
        cam_uiud, output_scene_name
):
    poses_path = [pose_path for pose_path in natsorted(
        glob(os.path.join(poses_path, "**")))
        for cam in cam_uiud if cam in pose_path]
    depths_path = [fix_depth(load_image(depth_path)) for depth_path in
                   natsorted(glob(os.path.join(depths_path, "**")))
                   for cam in cam_uiud if cam in depth_path]
    rgbs_path = [rgb_path for rgb_path in natsorted(
        glob(os.path.join(rgbs_path, "**")))
        for cam in cam_uiud if cam in rgb_path]
    intrinsics = [fix_intrinsics(load_matrix_from_txt(
        intrinsic_path, shape=(3, 3))) for intrinsic_path in natsorted(
            glob(os.path.join(intrinsics_path, "**")))
            for cam in cam_uiud if cam in intrinsic_path]
    ...
    for i, (pose, depth, rgb, intrinsic) in enumerate(zip(
            poses_path, depths_path, rgbs_path, intrinsics)):
...
def process_scene(scene):
    ...
    house_segmentation_path = os.path.join(
        HOUSE_SEGMENTATIONS, scene, scene,
    ...
    region_to_camera_mapping = region_to_camera_mapping_fn(
        house_segmentation_path)

    for region in region_to_camera_mapping:
        print(region, len(region_to_camera_mapping[region]))
        region_to_camera_mapping[region] = list(set(
            region_to_camera_mapping[region]))
